...
class DevServer {
  ...
  private Handler staticJarHandler() throws MalformedURLException, IOException {
    ...
    URL url = BuckUtil.buckOutGen(root)
        .resolve("gitiles-servlet")
        .resolve("static-resources.zip")
        .toUri().toURL();
    rh.setBaseResource(Resource.newResource("jar:" + url + "!/+static"));
