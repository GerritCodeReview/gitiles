...
import com.google.gitiles.RawUrls.SignedRawUrl;
import com.google.gitiles.RawUrls.ValidatedAuthKey;
 ...
 *
 * <p>This servlet provides two functions:
 *
 * <p><b>On the standard Gitiles domain</b> the servlet accepts {@code /+raw/} requests and replies
 * with an uncacheable redirect to the raw content domain. Browsers follow this redirect to load the
 * potentially untrusted content from the associated raw content domain.
 *
 * <p><b>On the raw content domain</b> the servlet verifies the authentication key query parameter
 * and returns the content. If the key is invalid it redirects to the standard domain to construct
 * an updated key.
 *
 * <p>Embedding applications may need to recognize the request is arriving on the raw content domain
 * and verify the {@link RawUrls#PARAM_AUTHKEY} instead of running the standard user authentication.
 */
public class RawContentServlet extends HttpServlet {
  ...
  private void redirectToRawDomain(HttpServletResponse res, RawUrls rawUrls, GitilesView view)
    ...
    SignedRawUrl sig = rawUrls.createRawUrl(removeAuthKey(view));
    long now = System.currentTimeMillis();
    long expires = Math.max(now, sig.getExpires());

    res.setDateHeader(HttpHeaders.DATE, now);
    res.setDateHeader(HttpHeaders.EXPIRES, expires);
    res.setHeader(
        HttpHeaders.CACHE_CONTROL,
        String.format("private, no-store, must-revalidate, max-age=%d", (expires - now) / 1000L));
    res.sendRedirect(sig.getUrl());
  }

  private void doGetRawContent(
      HttpServletRequest req, HttpServletResponse res, RawUrls rawUrls, GitilesView view)
      throws IOException {
    List<String> authKey = view.getParameters().get(RawUrls.PARAM_AUTHKEY);
    if (!authKey.isEmpty()) {
      view = removeAuthKey(view);
    }

    ValidatedAuthKey key = rawUrls.validateAuthKey(view, Iterables.getFirst(authKey, null));
    if (key == null) {
      // An auth key is required. Redirect through the safe server for authorization.
    ...
    if (!viewHasCommit(view)) {
      // Disable caching by setting expires to 0 (an ancient time in the past).
      key = new ValidatedAuthKey(key.isFromKey(), 0);
    }

    try (RevWalk rw = new RevWalk(ServletUtils.getRepository(req));
        WalkResult wr = WalkResult.forPath(rw, view, false)) {
      if (wr == null) {
        addCachingHeaders(res, key);
        res.setStatus(SC_NOT_FOUND);
        return;
      }
  ...
  private static GitilesView removeAuthKey(GitilesView view) {
    ...
    b.getParams().removeAll(RawUrls.PARAM_AUTHKEY);
    return b.build();
  }

  private static void addHeaders(
      HttpServletResponse res, WalkResult wr, ValidatedAuthKey key, String etag) {
    addCachingHeaders(res, key);
  ...
  private static void addCachingHeaders(HttpServletResponse res, ValidatedAuthKey key) {
    long now = System.currentTimeMillis();
    long expires = Math.max(now, key.getExpires());
    long maxAge = (expires - now) / 1000L;
    String cacheControl =
        key.isFromKey()
            ? String.format("private, no-store, must-revalidate, max-age=%d", maxAge)
            : String.format("public, must-revalidate, max-age=%d", maxAge);
  ...
  private static void rawTree(HttpServletRequest req, HttpServletResponse res, WalkResult wr)
    ...
    try (OutputStream stream = output(req, res);
      ...
      TreeWalk tw = wr.tw;
      while (tw.next()) {
        // Match git ls-tree format, with leading 0 padding.
