...
class RevisionParser {
  ...
  private String getRedirectFor(String revisionExpression) {
    /*
    git help check-ref-format tells that a valid refname cannot contain ^, ~, or :
    A valid refname cannot contain @{. Repository.resolve looks for that string
    so precisely until thefirst index of ^, ~, :, or @{ is the refname
     */
    String regex = "\\^|~|:|@\\{";
    Pattern pattern = Pattern.compile(regex);
    Matcher matcher = pattern.matcher(revisionExpression);
    String refName =
        matcher.find() ? revisionExpression.substring(0, matcher.start()) : revisionExpression;
