...
public abstract class Renderer {
  ...
  public String render(String templateName, Map<String, ?> soyData) {
    return newRenderer(templateName).setData(soyData).render().get();
  }
  ...
  void render(
    ...
    byte[] data = newRenderer(templateName).setData(soyData).render().get().getBytes(UTF_8);
  ...
  OutputStream renderStreaming(
      ...
      throws IOException {
    String html = newRenderer(templateName).setData(soyData).render().get();
