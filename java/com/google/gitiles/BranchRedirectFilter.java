...
public class BranchRedirectFilter extends AbstractHttpFilter {
  ...
  private Optional<String> rewriteRevision(Repository repo, Revision rev, FormatType format) {
    ...
    if (name.contains("^")) {
      ...
      String branch = name.substring(0, idx);
      String tail = name.substring(idx);
      return getRedirectBranch(repo, branch, format).map(b -> b + tail);
    }
    ...
    return getRedirectBranch(repo, name, format);
  }
