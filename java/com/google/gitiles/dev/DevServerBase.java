...
/**
 * Common, servlet-flavour-neutral dev server wiring.
 *
 * <p>Everything here is transform-safe: a {@code javax.servlet -> jakarta.servlet} plus {@code
 * org.eclipse.jetty.ee8 -> org.eclipse.jetty.ee10} import rewrite produces the EE10 form of this
 * class verbatim. The single structural difference between the EE8 and EE10 Jetty servlet APIs --
 * how a {@link ServletContextHandler} is installed into the core handler tree -- is isolated behind
 * the abstract {@link #toCoreHandler(ServletContextHandler)} seam, which a string rewrite cannot
 * bridge. See {@code DevServer} (the EE8 overlay) and {@code dev/ee10/DevServer.java} (the EE10
 * overlay), and {@code tools/gitiles-ee10/README.md}.
 */
abstract class DevServerBase {
  private static final Logger log = LoggerFactory.getLogger(DevServerBase.class);
  ...
  DevServerBase(File cfgFile) throws IOException, ConfigInvalidException {
  ...
  /**
   * Install the servlet context handler into the server's core handler tree.
   *
   * <p>This is the one place where the EE8 and EE10 Jetty servlet APIs diverge structurally: the EE8
   * {@link ServletContextHandler} implements {@code Supplier<org.eclipse.jetty.server.Handler>} and
   * must be unwrapped via {@code get()}, whereas the EE10 {@link ServletContextHandler} is itself a
   * core {@code Handler}. A {@code javax->jakarta} / {@code ee8->ee10} import rewrite cannot bridge
   * that, so each flavour supplies its own override.
   */
  protected abstract Handler toCoreHandler(ServletContextHandler handler);
