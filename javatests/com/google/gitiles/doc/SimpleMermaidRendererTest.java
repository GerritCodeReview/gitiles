...
public class SimpleMermaidRendererTest {
  ...
  public void testSubgraphEdgeWithDirectionOverrideAndDynamicLabelSpacing() {
    ...
    assertThat(svgOpt.isPresent()).isTrue();
    SvgDoc doc = new SvgDoc(svgOpt.get());
    assertThat(doc.findText("Teddy Bear Adventure")).isNotNull();
    assertThat(doc.findText("Puppy Playground")).isNotNull();
    assertThat(doc.findText("Cozy Blanket")).isNotNull();
    assertThat(doc.findText("Sweet Dream")).isNotNull();
    assertThat(doc.findText("Morning Sun")).isNotNull();
    assertThat(doc.findText("Rubber Ball")).isNotNull();
    assertThat(doc.findText("Flying Frisbee")).isNotNull();
    assertThat(doc.findText("Green Lawn")).isNotNull();
    assertThat(doc.findText("Soft Fluffy Hug")).isNotNull();
    assertThat(doc.findText("Wake Up Early")).isNotNull();

    Element p1 = doc.findText("Cozy Blanket");
    Element q1 = doc.findText("Rubber Ball");
    assertThat(p1).isNotNull();
    assertThat(q1).isNotNull();
    double p1Y = Double.parseDouble(p1.getAttribute("y"));
    double q1Y = Double.parseDouble(q1.getAttribute("y"));
    assertThat(p1Y).isLessThan(q1Y);

    Element p2 = doc.findText("Sweet Dream");
    Element p3 = doc.findText("Morning Sun");
    assertThat(p2).isNotNull();
    assertThat(p3).isNotNull();
    double p1X = Double.parseDouble(p1.getAttribute("x"));
    double p2X = Double.parseDouble(p2.getAttribute("x"));
    double p3X = Double.parseDouble(p3.getAttribute("x"));
    assertThat(p1X).isLessThan(p2X);
    assertThat(p2X).isLessThan(p3X);

    assertThat(p2X - p1X).isGreaterThan(120.0);
  }
  ...
  public void testIsolatedSubgraphInHorizontalGraph() {
    ...
    SvgDoc doc = new SvgDoc(svgOpt.get());
    assertThat(doc.findText("Flower Garden")).isNotNull();
    assertThat(doc.findText("Bright Tulip")).isNotNull();
    assertThat(doc.findText("Daisy Flower")).isNotNull();
  }
  ...
  public void testNestedSubgraphsWithInternalSubgraphEdgeAndInheritedDirection() {
    ...
    assertThat(svgOpt.isPresent()).isTrue();
    SvgDoc doc = new SvgDoc(svgOpt.get());
    assertThat(doc.findText("Toy Warehouse")).isNotNull();
    assertThat(doc.findText("Teddy Room")).isNotNull();
    assertThat(doc.findText("Puppy Room")).isNotNull();
    assertThat(doc.findText("Brown Bear")).isNotNull();
    assertThat(doc.findText("Happy Dog")).isNotNull();
  }
}
