...
public class PathServletTest extends ServletTest {
  ...
  public void rootTreeHtml() throws Exception {
    ...
    Map<String, ?> data = serveData("/repo/+/master/");
  ...
  public void subTreeHtml() throws Exception {
    ...
    data = serveData("/repo/+/master/foo");
    assertEquals("TREE", data.get("type"));
  ...
  public void fileHtml() throws Exception {
    ...
    Map<String, ?> data = serveData("/repo/+/master/foo");
  ...
  public void symlinkHtml() throws Exception {
    ...
    Map<String, ?> data = serveData("/repo/+/master/bar");
  ...
  public void gitlinkHtml() throws Exception {
    ...
    Map<String, ?> data = serveData("/repo/+/master/gitiles");
  ...
  public void blobText() throws Exception {
    repo.branch("master").commit().add("foo", "contents").create();
    String text = serveText("/repo/+/master/foo", "100644");
  ...
  public void symlinkText() throws Exception {
    ...
    String text = serveText("/repo/+/master/baz", "120000");
  ...
  public void treeText() throws Exception {
    ...
    assertEquals(expected, decodeBase64(serveText("/repo/+/master/", "040000")));

    expected = "100644 blob " + blob.name() + "\tbar\n";
    assertEquals(expected, decodeBase64(serveText("/repo/+/master/foo", "040000")));
    assertEquals(expected, decodeBase64(serveText("/repo/+/master/foo/", "040000")));
  }
  ...
  public void treeTextEscaped() throws Exception {
    ...
    assertEquals("100644 blob " + blob.name() + "\t\"foo\\nbar\\rbaz\"\n",
        decodeBase64(serveText("/repo/+/master/", "040000")));
  }
  ...
  public void treeJson() throws Exception {
    ...
    Tree tree = serveJson("/repo/+/master/", Tree.class);
    ...
    tree = serveJson("/repo/+/master/foo", Tree.class);
  ...
  protected String serveText(String path, String expectedMode) throws Exception {
    FakeHttpServletResponse res = serveText(path);
