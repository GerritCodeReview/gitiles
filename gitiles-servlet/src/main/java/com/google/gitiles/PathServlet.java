...
public class PathServlet extends BaseServlet {
   ...
   */
  private static final Pattern VERBATIM_SUBMODULE_URL_PATTERN = Pattern.compile("^(" + Joiner.on(
      '|').join("https?://[^.]+.googlesource.com/.*", "https?://[^.]+.googlecode.com/.*",
      "https?://code.google.com/p/.*", "https?://github.com/.*") + ")$", Pattern.CASE_INSENSITIVE);

  static enum FileType {
    TREE(FileMode.TREE), SYMLINK(FileMode.SYMLINK), REGULAR_FILE(
        FileMode.REGULAR_FILE), EXECUTABLE_FILE(FileMode.EXECUTABLE_FILE), GITLINK(FileMode.GITLINK);
  ...
  private void showTree(HttpServletRequest req, HttpServletResponse res, RevWalk rw, TreeWalk tw,
    ...
    GitilesView view = ViewFilter.getView(req);
    if (view.getParameters().get("stay").isEmpty()) {
  ...
  private void showGitlink(HttpServletRequest req, HttpServletResponse res, RevWalk rw,
    ...
    render(req, res, "gitiles.pathDetail", ImmutableMap.of("title",
        view.getTreePath(), "breadcrumbs",
        view.getBreadcrumbs(hasSingleTree), "type",
        FileType.GITLINK.toString(), "data", data));
  }

  private static String resolveHttpUrl(String remoteUrl) {
    return VERBATIM_SUBMODULE_URL_PATTERN.matcher(remoteUrl).matches() ? remoteUrl
        : null;
  }
}
