...
public class Linkifier {
  ...
  public List<Map<String, Object>> linkify(HttpServletRequest req, String message) {
    ...
    CommentLinkInfo changeIds =
        new CommentLinkInfo("ChangeIdLinks", CHANGE_ID_PATTERN, baseGerritUrl + "#/q/$0,n,z", "");
    ...
    for (int index = 0; index < parsed.size(); index++) {
      if (parsed.get(index).size() == 1) {
        Matcher m = allPattern.matcher(parsed.get(index).get("text").toString());
        if (m.find()) {
          // Go through the individual regexps now for replacement.
          for (CommentLinkInfo cli : operationalCommentLinks) {
            String text = parsed.get(index).get("text").toString();
            List<Map<String, Object>> resultingReplacement = cli.replace(text);
            if (resultingReplacement != null) {
              parsed.remove(index);
              parsed.addAll(index, resultingReplacement);
              index += resultingReplacement.size() - 1;
            }
          }
        }
      }
