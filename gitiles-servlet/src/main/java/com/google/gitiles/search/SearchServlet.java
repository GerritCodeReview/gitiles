...
public class SearchServlet extends BaseServlet {
  ...
  protected void doGetHtml(HttpServletRequest req, HttpServletResponse res) throws IOException {
    ...
    String repoUrl = GitilesView.repositoryIndex().copyFrom(view).setRepositoryName(repoDescr.name).toUrl();
    String revisionUrl = GitilesView.revision().copyFrom(view).setRevision(view.getRevision()).toUrl();
    ...
    try (CloseableHttpClient httpclient = HttpClients.createDefault()) {
      ...
      try (CloseableHttpResponse response = httpclient.execute(post)) {
        if (response.getStatusLine().getStatusCode() != SC_OK) {
          log.error("Unexpected response from zoekt server: "
