...
public interface RawUrls {
   ...
   * parameter {@link #PARAM_AUTHKEY} created by a {@link UrlSigner}, granting
   * short term access to the raw resource without normal user authentication.
   *
  ...
  public static class SignedRawUrl {
     ...
     *
     * @param url complete URL of the raw file access; this must be on a
     *        different content domain.
     * @param expiresInMillis milliseconds since the epoch when {@code url}
     *        expires; {@code 0} indicates {@code url} expires "quickly".
     */
    ...
    /**
     * @return milliseconds since the epoch when this URL will expire and no
     *         longer be accepted. If 0 the URL is assumed to expire "quickly"
     *         (almost immediately).
     */
  ...
  public static class ValidatedAuthKey {
    ...
    /**
     * @return {@code true} if this result came from an authenticated token,
     *         which implies the caller had previously used authorization.
     *         {@code false} if no token was presented by the client, but the
     *         validater allows it anyway, as the resource is publicly
     *         accessible and does not require authorization.
     */
