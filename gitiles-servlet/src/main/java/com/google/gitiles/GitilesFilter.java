...
class GitilesFilter extends MetaFilter {
  ...
  public synchronized BaseServlet getDefaultHandler(GitilesView.Type view) {
    ...
    switch (view) {
      case HOST_INDEX:
        return new HostIndexServlet(accessFactory, renderer, dateFormatterBuilder, urls);
      case REPOSITORY_INDEX:
        return new RepositoryIndexServlet(accessFactory, renderer, dateFormatterBuilder, timeCache);
      case REFS:
        return new RefServlet(accessFactory, renderer, dateFormatterBuilder, timeCache);
      case REVISION:
        return new RevisionServlet(accessFactory, renderer, dateFormatterBuilder, linkifier());
      case PATH:
        return new PathServlet(accessFactory, renderer, dateFormatterBuilder, urls);
      case DIFF:
      ...
      case DESCRIBE:
        return new DescribeServlet(dateFormatterBuilder, accessFactory);
      case ARCHIVE:
        return new ArchiveServlet(dateFormatterBuilder, accessFactory);
      case BLAME:
