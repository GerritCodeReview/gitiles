...
public class MarkdownToHtml implements Visitor {
  ...
  private static ImageLoader newImageLoader(Builder b) {
    if (b.config != null && b.reader != null && b.root != null) {
      return new ImageLoader(b.view, b.config, b.reader, b.root);
    }
    return null;
  }
  ...
  private boolean canRender(IframeBlock node) {
    if (config != null) {
      if (config.allowAnyIFrame) {
        return true;
      }
      for (String m : config.allowIFrame) {
