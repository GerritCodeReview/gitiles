...
public class DivNode extends AbstractNode {
  ...
  DivNode(String style, List<Node> list) {
    ...
    if (list.size() == 1 && list.get(0) instanceof ParaNode) {
      // Pegdown helpfully wraps most children into a <p> node.
      // Hoist them directly into the div.
      this.children = ((ParaNode) list.get(0)).getChildren();
    } else {
      this.children = list;
    }
  }
