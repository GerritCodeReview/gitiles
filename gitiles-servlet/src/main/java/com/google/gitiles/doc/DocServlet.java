...
import com.google.gitiles.doc.html.HtmlFormatter;
...
public class DocServlet extends BaseServlet {
  ...
  protected void doGetHtml(HttpServletRequest req, HttpServletResponse res)
    ...
    try {
      ...
      RootNode doc = MarkdownHelper.parseFile(view, srcmd);
      if (doc == null) {
        res.setStatus(SC_INTERNAL_SERVER_ERROR);
        return;
      }

      RootNode nav = MarkdownHelper.parseFile(view, navmd);
      if (navmd != null && nav == null) {
        res.setStatus(SC_INTERNAL_SERVER_ERROR);
        return;
      }
  ...
  private String etag(SourceFile srcmd, SourceFile navmd) {
    ...
    h.putInt(ETAG_GEN);
    h.putBytes(renderer.getTemplateHash(SOY_FILE).asBytes());
    if (navmd != null) {
  ...
  private void showDoc(HttpServletRequest req, HttpServletResponse res,
    ...
    data.putAll(MarkdownHelper.banner(view, nav));
    ...
    HtmlFormatter fmt = new HtmlFormatter(view);
    data.put("navbarHtml", fmt.toSoyHtml(nav));
    data.put("bodyHtml", fmt.toSoyHtml(doc));
