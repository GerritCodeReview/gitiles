...
public class DocServlet extends BaseServlet {
  ...
  private void showDoc(HttpServletRequest req, HttpServletResponse res,
      final GitilesView view, RootNode nav, RootNode doc) throws IOException {
    Map<String, Object> data = new HashMap<>();
    data.putAll(Navbar.banner(view, nav));
    ...
    Function<String, String> resolver = new Function<String, String>() {
      ...
      public String apply(String path) {
        return GitilesView.doc().copyFrom(view).setPathPart(path).build().toUrl();
      }
    };
    data.put("navbarHtml", new MarkdownToHtml(resolver).toSoyHtml(nav));
    data.put("bodyHtml", new MarkdownToHtml(resolver).toSoyHtml(doc));
