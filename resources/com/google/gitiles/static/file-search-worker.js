...
class Index {
  constructor(paths) {
    ...
    // Both byte buffers were sized for the whole path text, since UTF-8 length
    // is not known until the encode. Trim to what was written: baseBuf holds
    // only basenames and dirBuf only the interned directories, so most of the
    // reservation is empty. On chromium/src this is the difference between an
    // 87 MB index and a 28 MB one. Same reason dirOff is trimmed below.
    baseBuf = baseBuf.slice(0, bp);
    dirBuf = dirBuf.slice(0, dp);
